...
uint8_t secure_boot_verify(uint8_t Application_selector){
	int8_t ret=0;
	uint32_t applicationExtensionAddress;

	/************** fetch needed data from memory****************/
	if(Application_selector == 0){
		applicationExtensionAddress=APPLICATION_EXENSIONS_ADDRESS_B1;

	}
	else{
		applicationExtensionAddress=APPLICATION_EXENSIONS_ADDRESS_B2;

	}


uint32_t ROOT_CERT_ADDRESS = applicationExtensionAddress ;
uint32_t SB_CERT_ADDRESS = applicationExtensionAddress + SB_CERT_ADDRESS_OFFSET;
uint32_t META_DATA_ADDRESS = applicationExtensionAddress + META_DATA_ADDRESS_OFFSET ;
uint32_t SIGNATURE_ADDRESS   = applicationExtensionAddress + SIGNATURE_ADDRESS_OFFSET;
uint32_t CERTIFICATES_METADATA_ADDRESS  = applicationExtensionAddress + CERTIFICATES_METADATA_ADDRESS_OFFEST;

uint32_t MAIN_APPLICATION_START_ADDRESS  = applicationExtensionAddress + MAIN_APPLICATION_START_ADDRESS_OFFSET;   //128k byte offset from Application_extensions_Address
...
void jump_to_application(uint32_t start_addr){
	...
	/* First, disable all IRQs */
	// ensure to __enable_irq() in the application main function
//	   HAL_Init();
...
//  __disable_irq();
//
//  // Configure the boot mode to the inactive bank
//  HAL_FLASH_OB_Unlock();
//	if (active_bank==1){
//		CLEAR_BIT(FLASH->OPTCR, FLASH_OPTCR_BFB2_Msk);
//	}else{
