...
String FS_filename = "update2.txt";

//should be getted from RTDB
String Firebase_filename = "updates/update2.txt";
...
void setup() {
  ...
  while(1){
    if (Firebase.ready() && !taskCompleted) {
      ...
      if (!Firebase.Storage.download(&fbdo, STORAGE_BUCKET_ID /* Firebase Storage bucket id */,Firebase_filename  /* path of remote file stored in the 
bucket */,FS_filename /* path to local file */, mem_storage_type_flash /* memory storage type, mem_storage_type_flash and mem_storage_type_sd */, 
...
void loop() {
  ...
  if (Serial.available() >0) {
        ...
        uint16_t count = 0;
        if (readedflag == flag) {
            //Serial.print("flag received\n");
            SPIFFS.begin(); // LittleFS

            File file = SPIFFS.open( "/"+FS_filename , "r"); // LittleFS
